...
def test_copay_only_when_category_has_copay(
    ...
    result = estimate_cost_share(ppo_gold, fresh_member, office_visit, in_network=True)
...
def test_deductible_absorbs_full_allowed_when_below_remaining(
    ...
    result = estimate_cost_share(ppo_gold, fresh_member, imaging_procedure, in_network=True)
...
def test_partial_deductible_then_coinsurance(
    ...
    result = estimate_cost_share(ppo_gold, fresh_member, surgery_procedure, in_network=True)
...
def test_deductible_already_met(ppo_gold: Plan, imaging_procedure: Procedure) -> None:
    met = Member(
        member_id="m_met",
        name="Met",
    ...
    result = estimate_cost_share(ppo_gold, met, imaging_procedure, in_network=True)
    assert result.deductible_applied_cents == 0
...
def test_out_of_network_uses_billed_amount(hdhp_silver: Plan, imaging_procedure: Procedure) -> None:
    member = Member(
        member_id="m_partial",
        name="Partial",
    ...
    result = estimate_cost_share(hdhp_silver, member, imaging_procedure, in_network=False)
...
def test_out_of_pocket_max_caps_member_share(
    ...
    result = estimate_cost_share(ppo_gold, near_oop_member, surgery_procedure, in_network=True)
...
def test_oop_max_can_absorb_into_copay(ppo_gold: Plan, office_visit: Procedure) -> None:
    ...
    result = estimate_cost_share(ppo_gold, almost_capped, office_visit, in_network=True)
...
def test_zero_remaining_oop_means_plan_pays_everything(
    ...
    result = estimate_cost_share(ppo_gold, capped, surgery_procedure, in_network=True)
...
def test_mismatched_plan_raises(
    ...
    with pytest.raises(ValueError, match="Member m1"):
        estimate_cost_share(hdhp_silver, fresh_member, office_visit, in_network=True)
...
def test_coinsurance_breakdown_sums_to_allowed(
    ...
    result = estimate_cost_share(hdhp_silver, member, odd_procedure, in_network=True)
