...
function sweepValuesFor(feature: SweepFeature): Array<number | string> {
  switch (feature) {
    ...
    case "sex":
      return ["female", "male"];
    case "region":
      return REGIONS as unknown as string[];
  }
}
...
export function PredictorPage() {
  ...
  const [planId, setPlanId] = useState<KnownPlanId | "none">("ppo_gold");
  ...
  const update = useCallback(
    <K extends keyof PredictionFeatures>(key: K, value: PredictionFeatures[K]) => {
    ...
    const handle = setTimeout(async () => {
      ...
      try {
        const r = await predict(
          features,
          planId === "none" ? undefined : planId
        );
        if (!cancelled) setResult(r);
      } catch (e) {
        if (!cancelled) setResultError(String(e));
      } finally {
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(async () => {
      ...
      setSweepError(null);
      try {
        const r = await whatif(
          features,
          sweepFeature,
          values,
          planId === "none" ? undefined : planId
        );
        if (!cancelled) setSweep(r);
      } catch (e) {
        if (!cancelled) setSweepError(String(e));
      } finally {
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              <p className="text-sm text-slate-600">
                Hold everything else fixed and vary one feature.
              </p>
            </div>
            <Select<SweepFeature>
              label="Vary"
